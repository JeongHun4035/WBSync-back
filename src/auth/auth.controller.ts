...
export class AuthController {
  ...
  signup(@Body() dto: SignUpDto) {
    ...
    if (!idRegex.test(id)) {
      throwBadRequest('ID는 영문, 숫자, 언더바(_)로 구성해주세요.', 'INVALID_ID_FORMAT');
    }
    ...
    if (!passwordRegex.test(password)) {
      throwBadRequest('비밀번호는 특수문자 포함 8 ~ 16 자로 지정해주세요.', 'INVALID_PASSWORD_FORMAT');
    }

    if (!recheckPassword || typeof recheckPassword !== 'string' || recheckPassword.trim() ==='') {
      throwBadRequest('비밀번호 재확인을 입력해주세요.', 'REQUIRED_RECHECK_PASSWORD');
    }

     if (recheckPassword !== password) {
      throwBadRequest('비밀번호 재확인이 일치하지않습니다.', 'NOT_CORRECT_PASSWORD');
    }
    ...
    if (!phoneRegex.test(phoneNumber)) {
      throwBadRequest('비상 연락망은 (-) 제외, 숫자만 입력해주세요.', 'INVALID_PHONENUMBER_FORMAT');
    }
